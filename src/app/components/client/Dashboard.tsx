...
import MyButton from "@/app/components/UI/button/MyButton";

import { Post } from "@/app/types";
import MyHeader from "@/app/components/UI/header/MyHeader";
import toast from "react-hot-toast";




export default function Dashboard({ startdb, validatedDB }: { startdb: any, validatedDB: any  }) {
  ...
  */
  ...
  useEffect(() => {
    if (startdb == 1) {
      toast.error("startsetup db error");
      console.log("toast started")
    }
    console.log("out of if after toast")
    if (validatedDB.error) {
      toast.error(validatedDB.error.message);
    }
  }, [])
  return (
    <div className="App">
